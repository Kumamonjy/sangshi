          ...
          <view class="stats-section">
            ...
            <view class="stats-grid">
              ...
              <view class="stat-item">
                ...
                <text class="stat-value">{{ currentTotalMoveRange }}<text class="stat-unit">格/秒</text></text>
              </view>
              <view class="stat-item">
                <text class="stat-icon">⚡</text>
                <text class="stat-value">{{ currentTotalAttackSpeed }}<text class="stat-unit">次/秒</text></text>
              </view>
              <view class="stat-item">
                <text class="stat-icon">🎯</text>
            ...
            <view class="ai-type-row">
              <text class="ai-type-label">行动逻辑</text>
              <text class="ai-type-tag" :style="{ color: currentAiType.color, borderColor: currentAiType.color }">{{ currentAiType.label }}</text>
            </view>
          </view>
              ...
              >
                ...
                <template v-if="currentCharacter?.equipment[key]">
                  ...
                  <view class="equip-effects" v-if="currentCharacter?.equipment[key]">
                    ...
                    <text v-if="getEquipmentStats(currentCharacter.equipment[key]!).moveSpeed" class="equip-effect">
                      移+{{ getEquipmentStats(currentCharacter.equipment[key]!).moveSpeed }}
                    </text>
                ...
                >
                  ...
                  <view class="equip-info">
                    ...
                    <view class="equip-effect-list">
                      ...
                      <text v-if="getEquipmentStats(item).moveSpeed" class="equip-effect">移动频率+{{ getEquipmentStats(item).moveSpeed }}</text>
      ...
      <template v-else>
        ...
        <view class="character-list">
          ...
          <view class="hire-stats">
            ...
            <view class="hire-stat">
              <text>⚡ {{ char.baseAttackSpeed.toFixed(2) }}/秒</text>
            </view>
            <view class="hire-stat">
              <text>👟 {{ formatMoveFreq(char.baseMoveSpeed) }}</text>
            </view>
          </view>
...
const currentTotalMoveRange = computed(() => currentCharacter.value?.moveSpeed ?? 0)

const currentTotalAttackSpeed = computed(() => currentCharacter.value?.attackSpeed ?? 0)

const currentTotalAttackRange = computed(() => currentCharacter.value?.attackRange ?? 0)

/** 根据 attackRange 推断行动逻辑标签（与 BattleManager.toSimChar 规则一致） */
function getAiTypeLabel(attackRange: number): { label: string; color: string } {
  if (attackRange <= 2) return { label: '缠斗', color: '#f59e0b' }
  if (attackRange === 3) return { label: '远攻', color: '#3b82f6' }
  return { label: '狙击', color: '#8b5cf6' }  // >3
}

/** 格式化移动频率：moveSpeed (格/秒) → 每 X 秒走 1 格 */
...
const currentAiType = computed(() => getAiTypeLabel(currentTotalAttackRange.value))
...
function getHireCost(char: typeof HIREABLE_CHARACTERS[0]): number {
  // moveSpeed 范围 0.01~1.25（格/秒），系数 200 保持与原回合制合理费用相当
  return Math.ceil(char.maxHp + char.maxMp + 5 * char.attack + 5 * char.defense + 200 * char.moveSpeed + 100 * char.attackRange + 300 * char.baseAttackSpeed)
}
