          ...
          >
            ...
            <view class="item-info">
              ...
              <view class="item-effect-list">
                ...
                <text v-if="getEquipmentStats(item).moveSpeed" class="item-effect">移动频率+{{ getEquipmentStats(item).moveSpeed }}</text>
...
function getSellPrice(item: any): number {
  ...
  const total = (item.baseStats.attack || 0) + (item.baseStats.defense || 0) + (item.baseStats.hp || 0) + (item.baseStats.mp || 0) + (item.baseStats.moveSpeed || 0) + (item.baseStats.attackRange || 0)
