...
export interface StatusEffect {
  ...
  hpRegenPercent?: number   // 每秒恢复生命值百分比
  mpRegenPercent?: number   // 每秒恢复法力值百分比
}
...
export interface SnowArea {
  ...
  expiresAfterPhase?: 'enemy' | 'player' // [废弃] 实时战斗不再按秒阶段清除
}
...
export interface Skill {
  ...
  power: number
  frequency: number // 冷却时间（秒），如 6 表示 6 秒可释放 1 次
  description?: string
  ...
  statusEffectDuration?: number // 状态效果持续时间（秒），0表示永久
  statusEffects?: StatusType[] // 技能对目标施加的多个状态效果
  statusEffectsDurations?: number[] // 对应statusEffects的持续时间（秒）
...
export interface Character {
  ...
  attribute?: Attribute
  aiType?: 'ranged' | 'sniper' | 'skirmisher' // 远攻/狙击/缠斗，默认按 attackRange 自动推断
}
...
export interface BattleCharacter {
  ...
  skillLastUsedTime?: Record<string, number> // key: skill id, value: 上次使用时间戳(ms)
  ...
  moveSpeed: number // 带装备加成的移动速度（格/秒）
  attackRange: number // 带装备加成的攻击范围（格子数）
  attackSpeed: number // 带装备加成的攻击速度（次/秒）
  ...
  targetCharacterId?: string // 当前锁定的攻击目标
  path?: { row: number; col: number }[] // 当前寻路路径
  stuckCounter?: number // 连续卡住的 tick 数
}
...
export function processEquipmentEffects(equipment: Equipment | null | undefined): EquipmentEffects {
  ...
  for (const item of slots) {
    ...
    if (stats.moveSpeed) result.moveSpeed += stats.moveSpeed
    if (stats.attackRange) result.attackRange += stats.attackRange
    if (stats.attackSpeed) result.attackSpeed += stats.attackSpeed
...
export function calculateCharacterStats(char: {
  ...
  return {
    ...
    moveSpeed: char.moveSpeed + equipEffects.moveSpeed,
    attackRange: char.attackRange + equipEffects.attackRange,
    attackSpeed: char.attackSpeed + equipEffects.attackSpeed
  }
}
...
export const SKILL_TEMPLATES: Record<string, Skill> = {
  ...
  hong_gai_mi_zong: { id: 'hong_gai_mi_zong', name: '红盖迷踪', mpCost: 40, type: 'heal', power: 0, frequency: 8, range: 1, description: '选择自身为目标，恢复自身10%生命值和10%法力值，驱散随机1个负面状态', attribute: 'yin', category: 'heal', skillTypeTag: '治疗', rangeTag: '1格', targetCountTag: '1个', selfHealPct: 0.10, selfMpHealPct: 0.10, dispelRandomDebuffs: 1 },
  // 杀生樱技能
  ...
  ming_qu_gui_zhen: { id: 'ming_qu_gui_zhen', name: '冥躯归真', mpCost: 100, type: 'heal', power: 0, frequency: 8, range: 1, description: '选择自身为目标，恢复自身30%的生命值和10%的法力值', effectType: 'shadow', attribute: 'yin', category: 'heal', skillTypeTag: '治疗', rangeTag: '1格', targetCountTag: '1个', selfHealPct: 0.30, selfMpHealPct: 0.10 },
