  ...
  <view class="battle-container">
    <view class="battle-header">
      <view class="turn-info">
        <text class="turn-label">战斗时长</text>
        <text class="turn-number">{{ formatBattleTime }}</text>
      </view>
      <view class="weather-info">
        <text class="weather-icon">{{ weatherIcon }}</text>
        <text class="weather-text">{{ weatherText }}</text>
      </view>
      <view class="phase-badge" :class="{ paused: gameStore.battleMap?.paused, 'speed-2': gameStore.battleMap?.speedMultiplier === 2, 'speed-3': gameStore.battleMap?.speedMultiplier === 3 }">
        {{ gameStore.battleMap?.paused ? '⏸ 已暂停' : `⚔️ ${gameStore.battleMap?.speedMultiplier || 1}x` }}
      </view>
    ...
    <view class="bottom-buttons" v-if="!gameStore.isSelectingGatherPoints">
      ...
      <view class="bottom-btn" @click="toggleBattlePause">
        <text>{{ gameStore.battleMap?.paused ? '▶ 继续' : '⏸ 暂停' }}</text>
      </view>
      <view class="bottom-btn speed-btn" @click="cycleBattleSpeed">
        <text>速度 {{ gameStore.battleMap?.speedMultiplier || 1 }}x</text>
      </view>
    </view>
...
function getBuildingAt(row: number, col: number): any | null {
  const map = gameStore.battleMap
  if (!map) return null
...
function getCharacterDefense(char: BattleCharacter): number {
  ...
  // 实时战斗已废弃防御姿态，此条件恒为 false
  if (false) { /* @deprecated was: char.isDefending */
...
function handleCellClick(row: number, col: number) {
  ...
  // @deprecated 实时战斗废弃阶段检查，任何玩家角色都可被选中（手动交互按钮已禁用）
...
function showBuildingStatPanel(building: any) {
...
function selectSkill(skill: Skill) {
  if (!selectedCharacter.value || selectedCharacter.value.mp < skill.mpCost) return
  // HP限制检查（如腐蚀粘液）
...
function getSkillAttackTargets(char: BattleCharacter, skill: Skill): (BattleCharacter | any | any)[] {
  const targets: (BattleCharacter | any | any)[] = []
  const map = gameStore.battleMap
  if (!map) return targets
...
/** @deprecated 实时战斗废弃秒结束，由 BattleManager 自动驱动 */
...
function toggleSpeed() {
  /* @deprecated 实时战斗废弃旧加速，用 cycleBattleSpeed */
}

/** 循环切换速度倍率 1x → 2x → 3x → 1x */
function cycleBattleSpeed() {
  const current = gameStore.battleMap?.speedMultiplier || 1
  const next = current >= 3 ? 1 : current + 1
  gameStore.setBattleSpeed(next)
}

/** 暂停/恢复实时战斗 */
function toggleBattlePause() {
  gameStore.toggleBattlePause()
}
