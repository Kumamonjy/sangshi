  ...
  <view class="book-container">
    ...
    <scroll-view class="content" scroll-y>
          ...
          <view v-for="char in currentFactionCharacters" :key="char.id" class="character-card">
            ...
            <view class="stats-section">
              ...
              <view class="stats-grid">
                ...
                <view class="stat-item">
                  <text class="stat-label">攻击频率</text>
                  <text class="stat-value">{{ char.baseAttackSpeed.toFixed(2) }} /秒</text>
                </view>
                <view class="stat-item">
                  <text class="stat-label">移动频率</text>
                  <text class="stat-value">{{ formatMoveFreq(char.baseMoveSpeed) }}</text>
                </view>
                <view class="stat-item">
                  <text class="stat-label">行动逻辑</text>
                  <text class="stat-value" :style="{ color: getAiTypeLabel(char.baseAttackRange).color }">{{ getAiTypeLabel(char.baseAttackRange).label }}</text>
                </view>
              </view>
            </view>
      ...
      <view v-if="activeTab === 1" class="tab-content">
        <!-- @deprecated 实时战斗废弃建筑系统 -->
        <view class="building-list">
          <view class="building-card" style="text-align:center;padding:20px;">
            <text style="color:#999;">🏗️ 建筑系统已在实时战斗中移除</text>
          </view>
        </view>
      </view>
...
import {
  ...
  HIREABLE_CHARACTERS,
  INITIAL_CHARACTERS,
  // BUILDING_CONFIG, // @deprecated 实时战斗废弃
  EQUIPMENT_CONFIG,
  ITEM_CONFIG,
  ATTRIBUTE_CONFIG,
  STATUS_CONFIG,
