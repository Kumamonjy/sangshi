...
export const useGameStore = defineStore('game', () => {
  ...
  // mapShakeTick 交替奇偶，使 CSS 动画类名�?0/1 间切换，连续触发时动画能重新播放
  ...
  // 箭雨�?AOE 一波能�?150+，阈值提前以更早进入降级
  const EFFECT_COUNT_WARN = 40
  const EFFECT_COUNT_LIMIT = 70
  // 技能光效队列上限，超出时丢弃最旧的，防�?DOM 无限堆积
  const EFFECT_MAX_QUEUE = 160

  // 统一清理定时器：�?300ms 扫描一次所有特效数组，按时间戳批量清除过期特效
  ...
  // 当前特效负载等级�?=空闲 1=中载(粒子减半) 2=高载(不再生成粒子)
  ...
  const totalMoveRange = computed(() => currentCharacter.value?.moveSpeed || 0)
      ...
      try {
        ...
        if (parsed.player.homeGrid) {
          // 检查是否是简化格式（一维数组）还是旧格式（二维数组�?          if (Array.isArray(parsed.player.homeGrid[0])) {
            ...
            // 恢复 hp/mp 到满值，恢复 avatar，恢�?isPlayerOwned，恢�?attribute
  ...
  async function saveGame() {
    if (player.value) {
      ...
      simplifiedPlayer.characters.forEach((char: any) => {
        ...
        char.hp = char.maxHp
        char.mp = char.maxMp
        // 删除 avatar 字段，加载时通过 getAvatarPath �?id 重新生成
        delete char.avatar
        ...
        if (parsed.player.homeGrid) {
          // 检查是否是简化格式（一维数组）还是旧格式（二维数组�?          if (Array.isArray(parsed.player.homeGrid[0])) {
            ...
            parsed.player.characters = parsed.player.characters.map((char: any) => {
              ...
              // �?CHARACTER_SKILLS 装配技能：忽略存档中的 skills 字段
              char.skills = buildFullSkillsForCharacter(char.id, char.equipment)
  ...
  function handleSimBattleEvent(ev: SimBattleEvent, state: SimBattleState) {
    switch (ev.type) {
      case 'attack': {
        ...
        if (!target) break
        const attr = (attacker?.job as Attribute) || 'normal'
        ...
        // 文字日志（伤害/治疗）
        const getCharName = (c: typeof target) => {
    ...
    */
    ...
    // 按顺序放置玩家角�?    playerChars.forEach((char, index) => {
      if (index < validPositions.length) {
        ...
        console.log('添加玩家角色到战�?', char.id, char.name, char.avatar, '位置:', pos.row, pos.col, '防御�?', char.defense, '攻击�?', char.attack)
      } else {
        console.error('没有足够的空位放置角�?', char.name)
      }
    })
    ...
    // 放置选中的建�?    for (const buildingInfo of buildingsToSpawn) {
      ...
      while (!placed && attempts < 100) {
        ...
        if (false && tile && tile.terrain === 'empty' && !hasBuilding && !hasCharacter) { // @deprecated 实时战斗废弃建筑
          const buildingConfig = (null as any) // @deprecated
          const calculatedMaxHp = getBuildingHp(buildingConfig.maxHp)
          const newBuilding = {
            id: buildingInfo.id,
    ...
    const collectibleCount = 3 // 固定3�?    for (let i = 0; i < collectibleCount; i++) {
      ...
      while (!placed && attempts < 50) {
        ...
        if (tile && tile.terrain === 'empty' && !hasBuilding && !hasCharacter && !hasCollectible) {
          const collectibleConfig = (null as any) // @deprecated
          collectibles.push({
            id: `collect_${i}`,
            type: 'spirit_grass',
    ...
    battleManager = new BattleManager(
      battleMap.value!, players, enemies,
    ...
    // �?100ms �?BattleManager �?SimState 同步回响应式 battleMap
    ...
    battleSyncTimer = setInterval(() => {
      ...
      // 同步血�?状�?位置到原 BattleCharacter 对象（Vue 模板绑定了这些对象引用）
      ...
      for (let r = 0; r < height; r++) {
        for (let c = 0; c < width; c++) {
          battleMap.value.tiles[r][c].character = undefined
        }
      }
      // 把存活角色放回对�?tile
  ...
  async function endBattle(victory: boolean, isEscape: boolean = false) {
    ...
    if (!player.value || !battleMap.value) { console.log('[END] early return: missing state'); return }
    if (battleMap.value.battleEnded && !isEscape) { console.log('[END] early return: already ended'); return }
    battleMap.value.battleEnded = true;

    // ============ 在 stop() 之前从 BattleManager 同步最终战斗统计 ============
    if (battleManager) {
      const finalSimState = battleManager.getState()
      for (const simChar of finalSimState.chars) {
        const arr = simChar.isPlayer ? battleMap.value.players : battleMap.value.enemies
        let target = arr.find(c => c.id === simChar.id)
        // 如果已被移到 defeatedCharacters 也找一下
        if (!target) {
    ...
    // ======== �?try/catch 包裹核心逻辑，确�?battleResult 一定能设上触发结算弹窗 ========
    const finalResultType: 'victory' | 'defeat' | 'escape' = victory ? 'victory' : (isEscape ? 'escape' : 'defeat')
    let resultData: any = {
    ...
    if (victory) {
      ...
      */
      ...
      // 7. 计算经验奖励：每个参战角色获�?敌人数量*10*敌人等级 + 摧毁建筑数量*30*敌人等级
    ...
    */
    ...
    resultData = {
      ...
      console.error('[END] 结算逻辑异常（已用默认值兜底）:', err)
    }

    // 无论前面是否报错，都确保 battleResult 被设�?�?触发结算弹窗
    console.log('[END] setting battleResult:', resultData.type)
  ...
  // 天下市集：购买装�?  async function buyShopEquipment(template: any): Promise<boolean> {
    ...
    // 计算 rarity：根据属性总数值决定（�?RARITY_CONFIG 键名一致）
    let rarity = 'common'
  ...
  function calculateVisibility() {
    ...
    for (const char of map.players) {
      if (char.hp > 0) {
        const visionRange = Math.max(char.attackRange || 1, char.moveSpeed || 1) + 1
  ...
  function isCellVisibleToActor(actor: BattleCharacter, row: number, col: number): boolean {
    if (!battleMap.value || !battleMap.value.visibilityEnabled) return true
    const visionRange = 'moveRange' in actor
      ? (Math.max(actor.attackRange || 1, actor.moveSpeed || 1) + 1)
      : 4
    const dist = Math.abs(actor.row - row) + Math.abs(actor.col - col)
    return dist <= visionRange
  }

  function filterVisibleTargets<T extends { row: number; col: number }>(actor: BattleCharacter, targets: T[]): T[] {
  ...
  // 统一创建战斗角色的函�?  function createBattleCharacter(
    ...
    const attackSpeed = template.attackSpeed !== undefined ? template.attackSpeed : 1
    
    const skillLastUsedTime: Record<string, number> = {}
  ...
  // 汇总角色当前所有状态对移动范围的影响（例如迅捷+1，瘸�?1�?  function getStatusMoveRange(char: BattleCharacter): number {
    ...
    for (const s of char.statuses) {
      const key = typeof s === 'object' && s.type ? s.type : s;
      const cfg = STATUS_CONFIG[key as StatusType];
      if (cfg?.effects?.moveSpeed) total += cfg.effects.moveSpeed;
    }
    return total;
  }
  ...
  function computeDefensePower(target: BattleCharacter): number {
    ...
    // 阵营防御加成：灵�?=60�?20%，灵�?=100�?40%，煞�?=100时额�?10%
  ...
  /* @deprecated 实时战斗废弃 �?状态结算已�?BattleManager �?applyStatusTick() 每秒执行，此处不再调�?*/
  // 秒结束时触发的状态：燃烧（扣10%最大生命�?5%最大法力值）、流血（扣12%最大生命值）
  ...
  function useSkill(skillId: string, attackerId: string, targetId?: string | string[]): boolean {
    ...
    if (skill.category === '陷阵') {
      ...
      if (result) {
        ...
        // processXianZhenSkill 内部已处�?MP 退还和日志，直接返�?false
        return false
      }
    }
    ...
    // 特殊处理【天寒地冻】技�?    else if (skillId === 'tian_han_di_dong') {
      ...
      if (targetId) {
        ...
        if (damageTargets.length > 0) {
          ...
          // 2. 在目标脚下产生雪地，持续1秒
          // 玩家使用：雪地持续到下一次敌方秒结�?�?expiresAfterPhase = 'enemy'
          // 敌方使用：雪地持续到下一次玩家秒结�?�?expiresAfterPhase = 'player'
  ...
  // 辅助函数：移除角色并保存�?defeatedCharacters 列表
  ...
  function calculateSkillDamage(attacker: BattleCharacter, target: BattleCharacter | null, skill: Skill): number {
    if (!battleMap.value) return 0
  ...
  function getBestSkillTargets(char: BattleCharacter, skill: Skill): BattleCharacter[] {
    if (!battleMap.value) return []
    ...
    const targetsWithDamage = validTargets.map((target: any) => {
      ...
      return { target: target as BattleCharacter, damage }
    })
  ...
  async function executeAttackMode(char: BattleCharacter) {
    ...
    let bestTarget: BattleCharacter | null = null
      ...
      if (!char.hasActed) {
        if (bestSkill) {
          // 对于直线/横扫攻击技能，传递方向参�?          if ((bestSkill.category === '直线' || bestSkill.category === '横扫') && bestTarget && 'direction' in bestTarget) {
            ...
            // 轰炸类AOE技能：选择伤害最大的格子为中心，传�?pos_row_col
            ...
            // 冰封之门：选择 range 范围内的最�?targetCount 个空格，�?pos_row_col 数组
            ...
            // 凌云飞剑/墨羽鹤翎：传递最�?个目标ID数组，对敌方目标造成伤害
            const skillTargets = getSkillAttackTargets(char, bestSkill)
            const validTargets = skillTargets.filter(t => !t.isObstacle)
            if (validTargets.length > 0) {
              const count = Math.min(3, validTargets.length)
    ...
    if (!char.hasActed) {
      ...
      let nearestTarget: BattleCharacter | null = null
      let minDistance = Infinity
  ...
  function getBuilding4AdjacentEmptyPositions(building): { row: number; col: number }[] {
    if (!battleMap.value) return []
  ...
  function getBuildingAdjacentEmptyPositions(building): { row: number; col: number }[] {
    if (!battleMap.value) return []
    ...
    battleMap.value.buildings.forEach(building => {
      ...
      if (building.type === 'spiritField' && building.isPlayer) {
        ...
        // 只在�?秒生成一�?        if (!building.hasSpawnedBonus && config.spawnRound && currentTurn === config.spawnRound) {
          ...
          if (emptyPositions.length > 0) {
            const itemConfig = (null as any) // @deprecated 实时战斗废弃.spirit_grass
            ...
            building.hasSpawnedBonus = true
            battleLog.value.push(`灵田�?个相邻空格上生成�?${count} 株灵草！`)
          }
        }
        return
      }
      ...
      if (building.type === 'elixirRoom' && building.isPlayer) {
        ...
        // 只在�?秒生成一�?        if (!building.hasSpawnedBonus && config.spawnRound && currentTurn === config.spawnRound) {
          ...
          if (emptyPositions.length > 0) {
            const itemConfig = (null as any) // @deprecated 实时战斗废弃.elixir
            const pos = emptyPositions[Math.floor(Math.random() * emptyPositions.length)]
            ...
            building.hasSpawnedBonus = true
            battleLog.value.push(`丹房�?个相邻空格上生成了一个丹药！`)
          }
        }
        return
      }
  ...
  // ============ 实时战斗控制 API（暴露给 battle.vue�?============

  /** 暂停实时战斗 */
  function pauseBattle() {
    if (battleManager) battleManager.pause()
  }

  /** 恢复实时战斗 */
  function resumeBattle() {
    if (battleManager) battleManager.resume()
  }

  /** 切换暂停状�?*/
  function toggleBattlePause() {
    if (battleManager) battleManager.togglePause()
  }

  /** 设置速度倍率�?/2/3�?*/
  function setBattleSpeed(multiplier: number) {
    if (battleManager) battleManager.setSpeedMultiplier(multiplier)
  }

  // ============ 暂停指挥 API（玩家手动操作） ============

  /** 暂停时手动释放技能（player 角色�?*/
  function castPlayerSkill(charId: string, skillId: string, targetRow?: number, targetCol?: number): boolean {
    if (!battleManager) return false
    if (!battleManager.isPaused()) return false // 只能暂停时放
    const ok = battleManager.castSkillByPlayer(charId, skillId, targetRow, targetCol)
    if (ok) {
  ...
  /** 查询技能剩余冷却（毫秒�?*/
  function getSkillCooldownMs(charId: string, skillId: string): number {
    if (!battleManager) return -1
    return battleManager.getSkillCooldownFor(charId, skillId)
  }

  /** 检查技能是否就绪（冷却�?0 + MP 足够 + 没被沉默�?*/
  function isSkillReady(charId: string, skillId: string): boolean {
    if (!battleManager) return false
    const cd = battleManager.getSkillCooldownFor(charId, skillId)
    if (cd > 0) return false
    // 再查 MP �?�?getState �?SimChar
    const state = battleManager.getState()
    const simChar = state.chars.find(c => c.id === charId)
    if (!simChar || simChar.dead) return false
